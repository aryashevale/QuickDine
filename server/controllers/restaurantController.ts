...
import { Booking } from "../middlewares/Booking.js";
...
export const getRestaurants = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const restaurant = await getRestaurants.find(queryObj).sort(sortOption);
        res.json(restaurant)
...
export const getFeaturedRestaurants = async (req: Request, res: Response): Promise<void> => {
    try {
        const featured = await getRestaurants.find({
            status: "approved",
        ...
        res.status(400).json({message: "Servor error"});
    }
}
    ...
    try {
        ...
        if(restaurant.status !== "approved"){
            ...
            if(req.headers.authorization && req.headers.authorization.startsWith("Bearer")){
                try {
                    const token = req.headers.authorization.startsWith(" ")[1];
...
export const getRestaurantAvailability = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const availability = restaurant.availableSlots.map((slot)=>{
            const bookedSeats = bookings.filter(()=>b.time === slot).reduce((sum, b) => sum+
         b.guests, 0)
